...
export default function AboutUsDashboardPage() {
  ...
  const openEdit = (item: AboutUsItem) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit ? `/api/backend/AboutUs/${editingId}` : '/api/backend/AboutUs';
      ...
      if (!response.ok) {
        const errorBody = await response.text().catch(() => '');
        console.error('Save AboutUs error:', response.status, errorBody.slice(0, 500));
        throw new Error('Failed to save AboutUs');
      }

      toast.success(isEdit ? 'تم تحديث محتوى عن جريدة تجديد' : 'تم إنشاء محتوى عن جريدة تجديد');
      closeForm();
      await fetchItems();
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/backend/AboutUs/${id}`, { method: 'DELETE' });

      if (!response.ok) {
        throw new Error('Failed to delete AboutUs');
      }
    ...
    <div>
      <div className="sm:flex sm:items-center sm:justify-between mb-6">
        <div className="sm:flex-auto text-right">
          ...
          <p className="mt-1 text-sm text-gray-500">إنشاء وتعديل وحذف محتوى صفحة عن جريدة تجديد</p>
        </div>
      ...
      {showForm && (
        <div className="mb-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 text-right mb-4">
            {editingId !== null ? 'تعديل محتوى عن جريدة تجديد' : 'إضافة محتوى جديد'}
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="اكتب محتوى صفحة عن جريدة تجديد هنا..."
                dir="rtl"
              />
            </div>
